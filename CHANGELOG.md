...
- When the signed-in user has only one incoming dataset, it's now shown as `Uploading directly to EMBER Dandiset 000xyz, "Incoming: ..."` with a link out to its archive view, instead of a disabled single-option dropdown ([#20](https://github.com/brain-bbqs/ember-uploader/pull/20))
