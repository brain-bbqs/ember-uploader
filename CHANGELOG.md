...
- Fixed the broken `npm run lint` script by adding the missing eslint config (typescript-eslint recommended, passing clean) at `configs/eslint.config.js`, wired a Lint step into CI, and added an isolated-environment eslint pre-commit hook via `mirrors-eslint` ([#36](https://github.com/brain-bbqs/bbqs-uploader/pull/36))
